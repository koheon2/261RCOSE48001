...
| `institution_name_matches` | publication-time 기관명 canonical/ROR 매핑 |
...
`institution_name_matches`는 원본 affiliation 문자열을 수정하지 않고 `(institution_name, country_code)` 단위로 canonical institution name, ROR ID, OpenAlex institution ID를 매핑합니다.
...
## Quality-Filtered Product Layer

제품 API의 country/institution/topic/trending/progress 지표는 conservative v0 품질 필터를 적용한 summary table을 사용합니다. 원본 `papers` row는 삭제하거나 수정하지 않고, `paper_quality_flags`에 provenance를 남깁니다.

- `exclude`: 제품 지표에서 제외합니다.
- `warning`: 제품 지표에서는 유지하고 후속 보정/provenance 대상으로 남깁니다.
- 현재 policy: `conservative_v0`

현재 적용된 conservative v0 결과:

| 항목 | 값 |
| --- | ---: |
| excluded papers | `3,223,432` |
| quality flag rows | `16,725,095` |
...
`backfill_paper_quality_flags`는 idempotent합니다. 중간에 끊겨도 같은 명령을 다시 실행하면 중복 없이 재개됩니다. 특정 rule만 재실행할 수도 있습니다.

```bash
cd backend
.venv/bin/python -m scripts.backfill_paper_quality_flags --rules future_year,pre_1900_year
.venv/bin/python -m scripts.backfill_paper_quality_flags --rules missing_abstract
```

`missing_abstract`는 대량 warning rule이라 오래 걸릴 수 있습니다.

## Institution Normalization

기관 비교와 기관 리더보드는 publication-time affiliation의 raw 기관명을 바로 쓰지 않고, 로컬 OpenAlex institutions snapshot과 ROR dump로 만든 `institution_name_matches`를 거쳐 canonical institution 기준으로 집계합니다. 원본 `paper_author_affiliations.institution_name`은 보존합니다.
...
6. `paper_quality_flags` backfill
7. `institution_name_matches` backfill
8. summary table refresh (`publication_institution_field_stats` 포함)
9. validation/smoke test 실행
